...
function PaymentSuccessContent() {
  ...
  useEffect(() => {
    const processPayment = async () => {
      ...
      if (source === 'stripe' && payment_intent) {
        try {
          // Para Stripe, apenas confirmar se o pagamento foi bem-sucedido
          ...
          setError('Erro ao processar pagamento Stripe')
          setIsProcessing(false)
          return
        }
      }
          ...
          {isProcessing ? (
            ...
            <>
                  ...
                  <div className="text-center space-y-3">
                    <div className="flex items-center justify-center space-x-2">
                      <span className="text-green-300 font-bold text-lg">
                        {paymentData.source === 'stripe' ? `💳 ${t('payment.card')}` : '🅿️ PayPal'}
                      </span>
                    </div>
                    {paymentData.amount && (
              ...
              <div className="bg-blue-500/10 border border-blue-400/20 rounded-xl p-4 mb-6">
                <p className="text-blue-300 text-sm">
                  🔄 {t('paymentSuccess.autoRedirectSucess')}
                </p>
              </div>
              <button
                onClick={() => router.push('/')}
